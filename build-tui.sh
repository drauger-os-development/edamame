#!/bin/bash
VERSION=$(cat DEBIAN/edamame-tui.control | grep 'Version: ' | sed 's/Version: //g')
PAK=$(cat DEBIAN/edamame-tui.control | grep 'Package: ' | sed 's/Package: //g')
ARCH=$(cat DEBIAN/edamame-tui.control | grep 'Architecture: '| sed 's/Architecture: //g')
...
copy=$(<DEBIAN/edamame-tui.install)
for each in $copy; do
...
sed -i "s/<\!--python_vert-->/$py_vert/g" "$FOLDER/DEBIAN/edamame-tui.control"
sed -i "s/ , //g" "$FOLDER/DEBIAN/edamame-tui.control"
#build the shit
mv "$FOLDER/DEBIAN/edamame-tui.control" "$FOLDER/DEBIAN/control"
