...
"""
    ...
    def remove_launcher(USERNAME):
        ...
        try:
            os.remove("/home/live/Desktop/edamame.desktop")
        except FileNotFoundError:
            try:
                os.remove("/home/%s/Desktop/edamame.desktop" % (USERNAME))
            except FileNotFoundError:
                try:
                    rmtree("/home/%s/.config/xfce4/panel/launcher-3" % (USERNAME))
                except FileNotFoundError:
                    eprint("""Cannot find launcher for edamame.
...
def install(settings, distro):
    ...
    if "OEM" in settings.values():
        with open("/etc/edamame/oem-post-install.flag", "w") as file:
            file.write("")
